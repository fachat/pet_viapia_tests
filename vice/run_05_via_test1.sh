...
#
# The D64 disk image (populated with reference data by run_05_via_test1_gen.sh)
...
#           Reference data in the D64 image (run run_05_via_test1_gen.sh first)
# Usage: bash vice/run_05_via_test1.sh <d64_image> [extra VICE options]

D64="${1:?Usage: $0 <d64_image> [extra VICE options]}"
shift

if [ ! -f "$D64" ]; then
    echo "ERROR: $D64 not found. Run 'make gen5' first." >&2
    exit 1
fi
